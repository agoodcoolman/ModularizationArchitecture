...
public class WideRouter {
    ...
    boolean startAllRegisiterLocalRouter() {
        Logger.i(TAG, "startAllRegisiterLocalRouter()...");
        Set<String> strings = sLocalRouterClasses.keySet();
        Iterator<String> iterator = strings.iterator();
        while (iterator.hasNext()) {
            for (int i = 0; i < strings.size(); i++) {
                String next = iterator.next();
                ConnectServiceWrapper connectServiceWrapper = sLocalRouterClasses.get(next);
                Class<? extends LocalRouterConnectService> clazz = connectServiceWrapper.targetClass;
                if (null == clazz) {
                    return false;
                }
                Intent binderIntent = new Intent(mApplication, clazz);
                binderIntent.setData(Uri.parse(next));
                mApplication.startService(binderIntent);
            }
        }
        return true;
    }
    boolean connectLocalRouter(final String domain) {
        Logger.i(TAG, "connectLocalRouter()...");
        ConnectServiceWrapper connectServiceWrapper = sLocalRouterClasses.get(domain);
        if(null == connectServiceWrapper){
            return false;
        }
